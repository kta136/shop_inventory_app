...
class SalesManager {
  ...
  Future<SaleRecord> createSaleRecord({
    ...
    final List<Future<bool>> stockDecreaseFutures = []; // To track stock decreases

    final String recordId = _uuid.v4(); // Generate ID for the SaleRecord
    final DateTime processedTimestamp = DateTime.now(); // Timestamp for processing
    ...
    for (final itemInput in itemsToSell) {
      if (itemInput.quantity <= 0 || itemInput.unitPrice < 0) {
         throw ArgumentError('Invalid quantity or price for product ID ${itemInput.productId}.');
      }

      // Find the product using InventoryManager to get latest details
      final Product? product = await _inventoryManager.getProductById(itemInput.productId);
     ...
     for (final item in preparedSaleItems) {
        bool stockDecreased = await _inventoryManager.decreaseStock(item.productId, item.quantity);
        if (!stockDecreased) {
     ...
     if (failedStockProductNames.isNotEmpty) {
        ...
        print("Sale aborted due to insufficient stock for: ${failedStockProductNames.join(', ')}");
        throw Exception('Insufficient stock for: ${failedStockProductNames.join(', ')}');
     }
    ...
    try {
      ...
      print("Error saving SaleRecord ${newSaleRecord.recordId} to database: $e");
      ...
      throw Exception('Failed to save sale record to database after attempting stock decrease.');
    }
  }


  // --- Retrieve Sales History ---
  Future<List<SaleRecord>> getSalesHistory(DateTime startDate, DateTime endDate) async {
